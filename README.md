...
* [`src/priolist_reframe/styles.cljs`](src/priolist_reframe/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/priolist_reframe/`](src/priolist_reframe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/priolist_reframe/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `priolist-reframe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/priolist_reframe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns priolist-reframe.example
  (:require [priolist-reframe.config :as config])
